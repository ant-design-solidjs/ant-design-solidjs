...
import type { InternalNamePath, InternalValidateOptions, RuleError, RuleObject, StoreValue } from '../interface.ts';
import { defaultValidateMessages } from './messages.ts';
    ...
    try {
        ...
        if (errObj.errors) {
            result = errObj.errors.map(({ message }) => {
                return message === CODE_LOGIC_ERROR ? messages.default : message;
            });
        }
    }
    ...
    const filledRules: RuleObject[] = rules
        .map((currentRule, ruleIndex) => {
            ...
            if (originValidatorFunc) {
                cloneRule.validator = (rule: RuleObject, val: StoreValue, callback: (error?: string) => void) => {
