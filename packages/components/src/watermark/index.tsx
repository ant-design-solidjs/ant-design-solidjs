...
const Watermark: Component<WatermarkProps> = props => {
    const {
        /**
    ...
    const markStyle = createMemo(() => {
        ...
        mergedStyle['background-position'] = `${positionLeft}px ${positionTop}px`;
    ...
    const [subElements, setSubElements] = createSignal(new Set<HTMLElement>());

    // Nest elements should also support watermark
    const targetElements = createMemo(() => {
    ...
    const [watermarkInfo, setWatermarkInfo] = createSignal<[base64: string, contentWidth: number]>(null!);
    ...
    createMutationObserver(() => targetElements(), { attributes: true, subtree: true }, onMutate);

    createEffect(syncWatermark);
    ...
    return (
        <div ref={setContainer} class={clsx(className, rootClassName)} style={{ position: 'relative', ...style }}>
            {childNode}
        </div>
    );
};
