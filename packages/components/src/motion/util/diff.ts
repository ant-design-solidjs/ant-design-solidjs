...
export function wrapKeyToObject(key: JSX.Key | KeyObject) {
    let keyObj: KeyObject;
    if (key && typeof key === 'object' && 'key' in key) {
        keyObj = key;
    } else {
        keyObj = { key: key as JSX.Key };
    }
    return {
        ...keyObj,
...
export function diffKeys(prevKeys: KeyObject[] = [], currentKeys: KeyObject[] = []) {
    let list: KeyObject[] = [];
    let currentIndex = 0;
    ...
    if (currentIndex < currentLen) {
        list = list.concat(currentKeyObjects.slice(currentIndex).map(obj => ({ ...obj, status: STATUS_ADD })));
    }
